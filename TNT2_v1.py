...
path="/Users/yasinceran/Library/CloudStorage/GoogleDrive-yaceran@gmail.com/My Drive/Research/glioblostama/TNT-2ndProject/sandstone_data_for_ML/full_labels_for_deep_learning/128_patches/"
imgpth=glob.glob(os.path.join(path, "*.tif"))
...
train_masks = [] 
for directory_path in glob.glob("full_labels_for_deep_learning/128_patches/masks/"):
...
trainpth=os.path.join(path, "images_as_128x128_patches.tif")
maskpth=os.path.join(path, "masks_as_128x128_patches.tif")
from PIL import Image


train_image = Image.open(trainpth)
train_mask = Image.open(maskpth)

h,w = np.shape(train_image)
train_images = np.zeros((train_image.n_frames,h,w,3))
train_masks = np.zeros((train_mask.n_frames,h,w))
for i in range(train_image.n_frames):
   train_image.seek(i)
   train_images[i,:,:,:] = np.array(train_image.convert(mode='RGB'))
   train_mask.seek(i)
   train_masks[i,:,:] = np.array(train_mask)
